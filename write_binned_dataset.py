...
def main() :
    ...
    dsec = DSEC(seqs=args.seqs, include_backward=True,
    ...
    dsec_written = DSEC(event_set=args.name,
                        seqs=args.seqs, include_backward=True,
                        num_bins=args.num_bins,
                        bin_type=args.bin_type)

    #e_live = dsec.prep_item(0, 186, True)['in']['events']
    #e_written = dsec_written.prep_item(0, 185, False)['in']['events']
    #e_written = dsec_written.prep_item(0, 186, True)['in']['events']


    """    for i in range(len(dsec) // 2 - 1) :
        print("forward backward test frame " + str(i))
        eflive_data = dsec.prep_item(0, i, False)
        eflive = eflive_data['in']['events']
        efliveback_data = dsec.prep_item(0, i+1, True)
        efliveback = efliveback_data['in']['events']

        assert efliveback_data['in']['dt'] == eflive_data['in']['dt']
        ...
        t_reconstruct -= efliveback_data['in']['dt']
        t_reconstruct *= -1
        ...
        efwrite = dsec_written.prep_item(0, i, False)['in']['events']
        efwriteback = dsec_written.prep_item(0, i+1, True)['in']['events']
    ...
    for i in range(len(dsec)) :
        ...
        if args.bin_type == 'sum' :
            if dsec.get_local_idx(i)[2] :
                ...
                e_live = e_live[sort_idx, :]
            # e_live = np.flip(e_live, axis=0)
        e_written = dsec_written[i]['in']['events']

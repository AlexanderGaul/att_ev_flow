...
from data.basic_datasets import CombinedProcessedSequence
...
class BasicArraySequenceRefactor(CombinedProcessedSequence) :
    def __init__(self,
                 dir:Path,
                 seq_idx=None,
                 backward_hack=False,
        ...
        self.dir = Path(dir)
        self.flow_sequence = BasicEventFlowSequence(self.dir / event_suffix,
                                                    self.dir / flow_suffix,
                                                    self.dir / ts_suffix)
        ...
        self.compute_volume = return_volume or unfold_volume or flat_volume or event_images

        prep = []
        if self.crop_t is not None :
            prep += [CropTimeFlowFrameEventData(self.crop_t)]
        if self.crop is not None :
            prep += [CropFlowFrameEventData(self.crop,
                                            offset='random'
                                            if self.random_crop_offset
                                            else 'center')]
        if self.backward_hack :
            prep.append(FlowFrameUniBackward())
        prep.append(EventData2EventArray())
        if not self.return_volume :
            prep.append(FlowFrame2Array())
        if self.compute_volume :
            prep.append(EventArray2Volume(self.t_bins))
            if self.backward_hack :
                prep.append(EventVolumeBackward())

            if self.unfold_volume :
                prep += [Volume2PatchArray(input_name='event_volume',
                                           output_name='event_array')]
            elif self.flat_volume :
                prep += [EventVolumeFlat()]
            elif self.event_images :
                prep += [EventVolume2ImagePair(),
                         ImagePair2PatchArray()]
        elif self.backward_hack :
            prep += [EventArrayBackward()]

        super().__init__([self.flow_sequence],
                         prep,
                         seq_idx)

    def get_item_id(self, idx) :
        if self.backward_hack :
            return (*super().get_item_id(idx), -1)
        else :
            return super().get_item_id(idx)

    def collate(self, items) :
        if not self.compute_volume :
            return collate_dict_list(items)
        else :
            return collate_dict_arrays(items)





class BasicArrayDatasetRefactor(Dataset) :
    def __init__(self, dir,
                 seqs=True,
                 batch_hacked_backward=False,
                 return_volume=False,
                 unfold_volume=False,
                 flat_volume=False,
                 event_images=False,
                 ...
                 random_crop_offset=False) :
        self.dir = Path(dir)
        self.seq_names = select(sorted(os.listdir(self.dir)), seqs)
        self.return_volume = return_volume
        self.unfold_volume = unfold_volume
        self.flat_volume = flat_volume
        self.event_images = event_images
        ...
        seq_params = (self.return_volume,
                      self.unfold_volume, self.flat_volume,
                      self.event_images,
                      self.t_bins,
                      self.crop_t,
                      self.crop,
        ...
        self.seqs = [BasicArraySequenceRefactor(self.dir / seq_name,
                                                i, False, *seq_params)
                     for i, seq_name in enumerate(self.seq_names)]
        self.cat_dataset = torch.utils.data.ConcatDataset(self.seqs)

        self.batch_hacked_backward = batch_hacked_backward
        if self.batch_hacked_backward :
            self.seqs_back = [BasicArraySequenceRefactor(self.dir / seq_name,
                                                         i, True, *seq_params)
                              for i, seq_name in enumerate(self.seq_names)]
    ...
    def __getitem__(self, idx) :
        ...
        if self.batch_hacked_backward :
            item = [item, self.cat_dataset_back[idx]]
        return item
...
class BasicVolumeSequence(Dataset) :
    def __init__(self, flow_sequence, event_sequence, num_bins) :
        raise DeprecationWarning("About to be removed")
        self.flow_sequence = flow_sequence
        self.event_sequence = event_sequence
...
class BasicDataset(Dataset) :
    def __init__(self, dir:Path, flow_suffix, ts_suffix, event_suffix,
        ...
        self.seqs = []
        for sq in seq_names :
            e_seq = ESeqClass(self.dir / sq / event_suffix)

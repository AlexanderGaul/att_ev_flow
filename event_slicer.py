...
class EventSlicer:
    def __init__(self, h5f: h5py.File, keep_file_open=False):
        ...
        self.t_offset = int(h5f['t_offset'][()]) if 't_offset' in self.h5f else 0
    ...
    def open_file(self) :
        self.h5f = h5py.File(self.fname, 'r')
        for dset_str in ['p', 'x', 'y', 't']:
            self.events[dset_str] = self.h5f['events/{}'.format(dset_str)]
    ...
    def get_events(self, t_start_us: int, t_end_us: int,
                   include_end: bool=False) -> Dict[str, np.ndarray]:
    ...
    def get_time_indices_offsets(
            ...
            time_start_us: int,
            time_end_us: int,
            include_end: bool = True) -> Tuple[int, int]:
        ...
        if not include_end and idx_end > 0:
            assert time_array[idx_end - 1] < time_end_us
        if include_end and idx_end > 0:
            assert time_array[idx_end - 1] <= time_end_us
